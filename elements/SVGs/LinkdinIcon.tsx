...
const LinkdeinIcon: React.FC<Props> = ({
  ...
  return (
    ...
    >
      <Circle cx="15.5" cy="15.5" r="15.5" fill="#E6F1FF" hovered={hovered} />
      <Path
...
const Circle = styled.circle<{ hovered: boolean }>`
  ...
  fill: ${({ hovered }) =>
    hovered ? "var(--primary-color-dark)" : "var(--primary-color-light)"};
`;
const Path = styled.path<{ hovered: boolean }>`
  stroke: ${({ hovered }) => (hovered ? "white" : "var(--primary-color-dark)")};
`;
